...
.fi { max-width: 1200px; margin: 0 auto; padding: 4.5rem 5vw 2rem; }

.ft-top { display: flex; justify-content: space-between; gap: 3rem; flex-wrap: wrap; margin-bottom: 3.5rem; }
.ft-brand { max-width: 240px; }
.fb-logo { font-family: 'Outfit', sans-serif; font-size: 1.4rem; font-weight: 900; color: var(--text); text-decoration: none; letter-spacing: -.04em; display: block; margin-bottom: .7rem; }
.dot { color: var(--red); }
.ft-brand p { font-size: .8rem; color: var(--muted); line-height: 1.7; }
.fb-badge { display: inline-flex; align-items: center; gap: .45rem; margin-top: 1rem; font-size: .72rem; color: var(--muted); background: var(--card); border: 1px solid var(--card-b); padding: .26rem .75rem; border-radius: 100px; }
...
.fn-head { font-size: .65rem; font-weight: 700; text-transform: uppercase; letter-spacing: .16em; color: var(--muted); margin-bottom: .35rem; }
...
.ft-email-block { margin-bottom: 3rem; }
.feb-label { display: block; font-size: .64rem; color: var(--muted); text-transform: uppercase; letter-spacing: .18em; margin-bottom: .55rem; }
.feb-email { display: inline-flex; align-items: center; gap: .7rem; font-family: 'Outfit', sans-serif; font-size: clamp(1.2rem, 2.5vw, 2rem); font-weight: 800; color: var(--text); text-decoration: none; letter-spacing: -.03em; border-bottom: 1px solid var(--card-b); padding-bottom: .2rem; transition: color .2s, border-color .2s, gap .2s; }
.feb-email:hover { color: var(--red); border-color: var(--red); gap: 1rem; }
.feb-email svg { transition: transform .2s; }
.feb-email:hover svg { transform: translate(3px,-3px); }

.ft-hr { height: 1px; background: var(--card-b); margin-bottom: 1.6rem; position: relative; overflow: hidden; }
.ft-hr::after { content: ''; position: absolute; inset-block: 0; left: 0; width: 22%; background: linear-gradient(90deg, var(--red), transparent); }

.ft-bottom { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; font-size: .74rem; color: var(--muted); }
.ft-icons { display: flex; gap: .45rem; }
.ft-icons a { display: flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: 7px; background: var(--card); border: 1px solid var(--card-b); color: var(--muted2); text-decoration: none; transition: color .2s, border-color .2s; }
